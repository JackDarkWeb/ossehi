            ...
            <div class="container">
                <div class="row">
                    <div class="col-lg-9 order-lg-last dashboard-content">
                        ...
                        <h2>{{ __('The products of your store') }}</h2>
                                        ...
                                        <div class="product-details">
                                            ...
                                            <form method="#" action="#">
                                                ...
                                                <div class="product-action">
                                                    ...
                                                    <a href="{{ route('destroy.store_product', ['lang' => app()->getLocale(), 'product_id' => $product->id, 'slug' => $product->slug]) }}" onclick="return confirm(`{{ __('Do you want to delete the product') }} ?`)" class="mx-4 destroy-store-product" title="{{ __('Delete store') }}"><i class="far fa-trash-alt text-danger" style="font-size: 1.9rem"></i></a>

                                                    <span id="btn-cancel-discount{{ $product->id }}">
                                                        @if($product->discount_price)
                                                            <span class="btn-icon btn-add-cart cancel-discount"  title="{{ __('Cancel discount') }}" data-action="{{ route('cancel.discount.store_product', ['lang' => app()->getLocale(), 'product_id' => $product->id, 'slug' => $product->slug]) }}"><i class="fas fa-percent"></i>{{strtoupper(__('Cancel discount'))}}</span>
                                                        @else
                                                            <a href="{{ route('discount.store_product', ['lang' => app()->getLocale(), 'product_id' => $product->id, 'slug' => $product->slug, 'price' => $product->price, 'devise' => $product->devise]) }}" id="discount{{ $product->id }}" class="btn-quickview d-none"  title="{{ __('Add discount') }}"><i class="fas fa-car"></i></a>
                                                    ...
                                                    <a href="{{ route('edit.store_product', ['lang' => app()->getLocale(), 'product_id' => $product->id, 'slug' => $product->slug]) }}" class="btn-quickview d-none" id="edit{{ $product->id }}" title="{{ __('Edit product') }}"><i class="fas fa-percent"></i>{{strtoupper(__('Edit product'))}}</a>

                                                </div>

                                                <div class="product-action">
                                                    <a href="{{ route_name('gallery.store_product', ['slug' => $product->slug]) }}" class="d-none" id="gallery{{$product->id}}"></a>
