<div class="product-single-container product-single-default product-quick-view container">
    ...
    <div class="container">
        ...
        <form method="post" action="{{ route('update.store_product', ['lang' => app()->getLocale(), 'product_id' => $product->id, 'slug' => $product->slug]) }}" class="mb-1" id="edit-store-product-form">
                    ...
                    <div class="select-custo mb-2">
                        ...
                        <select name="colors" id="colors" class="form-control form-control-sm"  multiple>
                            ...
                            @foreach(stock_colors() as $key => $value)
                                <option value="{{ $value }}" {{ ($product->colors_store_product && in_array($value, $product->colors_store_product) ) ? 'selected' : '' }}>{{ strtoupper(__( $key)) }}</option>
                            @endforeach
                        </select>
                    </div><!-- End .select-custom -->
                    ...
                    <div class="select-custo mb-2">
                        ...
                        <select name="sizes" id="sizes" class="form-control form-control-sm"  multiple>
                            ...
                            @foreach(stock_sizes() as $size)
                                <option value="{{ $size }}" {{ ($product->sizes_store_product && in_array($size, $product->sizes_store_product) ) ? 'selected' : '' }}>{{ strtoupper($size) }}</option>
                            @endforeach
                    ...
                    <div class="select-custo mb-2">
                        ...
                        <select name="Brands" id="brands" class="form-control form-control-sm"  multiple>
                            ...
                            @foreach(stock_brands() as $brand)
                                <option value="{{ $brand }}" {{ ($product->brands_store_product && in_array($brand, $product->brands_store_product) )? 'selected' : '' }}>{{ strtoupper($brand) }}</option>
                            @endforeach
