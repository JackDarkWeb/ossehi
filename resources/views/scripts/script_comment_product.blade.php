    ...
    $(function () {
        ...
        $doc.on('submit', '#comment-form', function (event) {
            ...
            if (Validator.getValue(this, '#comment')){
                ...
                Validator.requestAjax('POST', "{{ route_name('comments.product') }}", func_callback, request);
        ...
        function fetchLatestComments() {
            ...
            Validator.requestAjax('GET', "{{ route('comments.product.fetch', ['lang' => app()->getLocale(), 'product_id' => $product->id]) }}", func_fetch_callback);
        }
